...
namespace KRE
{
	namespace SVG
	{
			...
			void apply_fill_color() const {
				if(fill_color_) {
					if(fill_color_->has_color()) {
						get_fill_color_stack().emplace(fill_color_->r(), fill_color_->g(), fill_color_->b(), int(alpha_*255));
			...
			void apply_stroke_color() const {
				if(stroke_color_) {
					if(stroke_color_->has_color()) {
						get_stroke_color_stack().emplace(stroke_color_->r(), stroke_color_->g(), stroke_color_->b(), int(alpha_*255));
		...
		{
		public:
			SVGElement(const ptree& svg_data) 
		...
		Parse::Parse(const std::string& filename)
		{
			...
			for(auto& node : pt) {
				if(node.first == "svg") {
					svg_data_.emplace_back(new SVGElement(node.second));
				}
			}
		}
