...
INC := -Isrc -Iinclude $(shell pkg-config --cflags x11 sdl2 glew SDL2_image SDL2_ttf libpng zlib cairo)
...
svg_parser: $(objects) $(ogl_objects) $(sdl_objects) $(ogl_fixed_objects)
	@echo "Linking : svg_parser"
	@$(CCACHE) $(CXX) \
		$(BASE_CXXFLAGS) $(LDFLAGS) $(CXXFLAGS) $(CPPFLAGS) $(INC) \
		$(objects) $(ogl_objects) $(sdl_objects) $(ogl_fixed_objects) -o svg_parser \
...
clean:
	rm -f src/*.o src/*.d *.o *.d svg_parser
